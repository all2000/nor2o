...
public class WordNetConnector extends Resource implements WordNetConstants {
    ...
    protected boolean isTermContainedInWords(Synset synset, String term) {
    	for (int i=0;i<synset.getWords().length;i++) {
    		//System.out.println("\t word " + synset.getWord(i).getLemma());
    ...
    protected String getSpecificRelation(IndexWord word1, String term1, IndexWord word2, String term2, POS[] posES) {
    	...
    	try {
	        ...
	        if (concept1Synsets != null && concept2Synsets != null) {
	        	...
	        	for (Relationship rel : relsSet) {
	        		System.out.println("rel desde " + rel.toString());
    ...
    protected boolean isHyponym(String term1, String term2, POS pos) {
    	...
    	IndexWord iw;
    	//System.out.println("isHypernym: term " + term1 + " POS "+ pos.getLabel());
		try {
    ...
    protected boolean isHypernym(String term1, String term2, POS pos) {
    	HashSet<String> res = new HashSet<String>();
    	//System.out.println("isHyponym: term " + term1 + " POS "+ pos.getLabel());
    	IndexWord iw;
		try {
			iw = wordnet.lookupIndexWord(pos, term1);
			if (iw==null)
    ...
    protected boolean isMeronym(String term1, String term2, POS pos) {
    	HashSet<String> res = new HashSet<String>();
    	//System.out.println("isMeronym: term " + term1 + " POS "+ pos.getLabel());
    	IndexWord iw;
		try {
			iw = wordnet.lookupIndexWord(pos, term1);
			if (iw==null)
					...
					for (int j=0; j<concept2iWord.length; j++) {
						if (concept1iWord[i].getSenseCount()!=0 && concept2iWord[j].getSenseCount()!=0) {
					    	...
					    	for(int p=0; p<list.size(); p++) {
						    	...
						    	System.out.println(ptnl.toString());
						    	for(int q=0; q<ptnl.getSize(); q++) {
						    		//PointerTargetNode ptn = (PointerTargetNode)ptnl.
						    		System.out.println(ptnl.getType());
						    		System.out.println(ptnl.getTargetSynset());
							...
							if (!list.isEmpty()) {
								...
								System.out.println(shall.toString());
								System.out.println(pt.getLabel());
							}
...
/*
		try {
			Runtime.getRuntime().exec("c:\\Users\\boricles\\Documents\\Proyectos\\patch_64b_update_issue.exe");
		} catch (IOException e) {
			// TODO Auto-generated catch block
	...
	*/	
	...
	WordNetConnector wc = new WordNetConnector();
		wc.init();
		//System.out.println(wc.getRelation("building", "hospital"));
		//System.out.println(wc.getRelation("finger", "body"));
		//System.out.println(wc.getRelation("body", "finger"));
		//System.out.println(wc.getRelation("keyboard", "key"));
		//System.out.println(wc.getRelation("family", "kid"));
		//System.out.println(wc.getRelation("blue", "color"));
		//System.out.println(wc.getRelation("Sterols", "Cholesterol"));
		//System.out.println(wc.getRelation("Sterols", "Cholesterol"));
		//System.out.println(wc.getRelation("chapter", "book"));
		//System.out.println(wc.getRelation("girl", "woman"));
		//System.out.println(wc.getRelation("car", "transport"));
		System.out.println(wc.getRelation("artist", "musician"));
		
		System.out.println(wc.getRelation("artist", "painter"));
		
		//System.out.println(wc.getRelation("hand", "finger"));
		/*
